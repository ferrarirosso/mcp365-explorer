...
import type { IMailExplorerProps } from './IMailExplorerProps';
import {
  McpBrowserClient,
  MCP_AUDIENCE
...
import {
  MAIL_SERVER_ID,
  MAIL_SERVER_EXAMPLES
} from '../services/McpServerCatalog';
import type { IToolExample } from '../services/McpServerCatalog';
import { MailShowcase } from './MailShowcase';

// ─── Preset Storage ─────────────────────────────────────────────────

const PRESETS_STORAGE_KEY = 'mcp365-mail-presets';
...
export const MailExplorer: React.FC<IMailExplorerProps> = (props) => {
  ...
  const selectedExamples: IToolExample[] = state.selectedToolName
    ? (MAIL_SERVER_EXAMPLES[state.selectedToolName] || [])
    : [];
  ...
  return (
    <div style={{ padding: 16, fontFamily: theme.fonts.medium.fontFamily }}>
      ...
      <Stack horizontal verticalAlign="center" tokens={{ childrenGap: 12 }} styles={{ root: { marginBottom: 16 } }}>
        <Icon iconName="Mail" styles={{ root: { fontSize: 24, color: theme.palette.themePrimary } }} />
        <Text variant="xLarge" styles={{ root: { fontWeight: 600 } }}>MCP365 Explorer: Outlook Mail</Text>
        <span style={{
          fontFamily: 'Consolas, Monaco, "Courier New", monospace',
          fontSize: 12,
          fontWeight: 600,
      ...
      <Pivot styles={{ root: { marginBottom: 16 } }} selectedKey={state.activeTab} onLinkClick={(item) => { if (item?.props.itemKey) setState(prev => ({ ...prev, activeTab: item.props.itemKey as string })); }}>
        ...
        {isConnected && clientRef.current && (
          <PivotItem headerText="Showcase" itemIcon="Rocket" itemKey="showcase">
            <MailShowcase client={clientRef.current} theme={theme} onLog={addLog} />
          </PivotItem>
        )}
