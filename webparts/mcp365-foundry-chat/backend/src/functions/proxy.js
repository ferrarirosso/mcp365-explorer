...
function rateLimitExceededResponse(request, errorMessage) {
  return { status: 429, headers: getCorsHeaders(request), jsonBody: { error: errorMessage } };
}
...
async function chatCompletionsHandler(request, context) {
  ...
  try {
    ...
    // Browser-facing headers: only correlation id + rate-limit hints. No
    // backend identity, deployment name, or auth method leaks.
