...
import * as strings from 'WordExplorerWebPartStrings';
import { WordExplorer } from './components/WordExplorer';
import type { IWordExplorerProps } from './components/IWordExplorerProps';

export interface IWordExplorerWebPartProps {
  environmentId: string;
}

export default class WordExplorerWebPart extends BaseClientSideWebPart<IWordExplorerWebPartProps> {
  private _isDarkTheme: boolean = false;
  private _tokenProvider: AadTokenProvider | undefined;

  public render(): void {
    const element: React.ReactElement<IWordExplorerProps> = React.createElement(
      WordExplorer,
