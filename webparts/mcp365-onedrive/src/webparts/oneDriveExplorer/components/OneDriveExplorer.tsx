...
import type { IOneDriveExplorerProps } from './IOneDriveExplorerProps';
import {
  McpBrowserClient,
  MCP_AUDIENCE
...
import {
  ONEDRIVE_SERVER_ID,
  ONEDRIVE_SERVER_EXAMPLES
} from '../services/McpServerCatalog';
import type { IToolExample } from '../services/McpServerCatalog';
import { OneDriveShowcase } from './OneDriveShowcase';

// ─── Preset Storage ─────────────────────────────────────────────────

const PRESETS_STORAGE_KEY = 'mcp365-onedrive-presets';
    ...
    if (key === 'content' && typeof input[key] === 'string') {
      ...
      if (content.indexOf('<html') !== -1 || content.indexOf('<div') !== -1) {
        ...
        const plain = content
          ...
          .replace(/\n{3,}/g, '\n\n')
          .trim();
        // Keep first meaningful part (before signature/boilerplate separators)
        const sepIdx = plain.indexOf('________________');
        result[key] = sepIdx > 0 ? plain.substring(0, sepIdx).trim() : plain;
        return;
      }
    }
...
export const OneDriveExplorer: React.FC<IOneDriveExplorerProps> = (props) => {
  ...
  const selectedExamples: IToolExample[] = state.selectedToolName
    ? (ONEDRIVE_SERVER_EXAMPLES[state.selectedToolName] || [])
    : [];
  ...
  return (
    <div style={{ padding: 16, fontFamily: theme.fonts.medium.fontFamily }}>
      ...
      <Stack horizontal verticalAlign="center" tokens={{ childrenGap: 12 }} styles={{ root: { marginBottom: 16 } }}>
        <Icon iconName="OneDrive" styles={{ root: { fontSize: 24, color: theme.palette.themePrimary } }} />
        <Text variant="xLarge" styles={{ root: { fontWeight: 600 } }}>MCP365 Explorer: Microsoft OneDrive</Text>
        <span style={{
          fontFamily: 'Consolas, Monaco, "Courier New", monospace',
          fontSize: 12,
          fontWeight: 600,
      ...
      <Pivot styles={{ root: { marginBottom: 16 } }} selectedKey={state.activeTab} onLinkClick={(item) => { if (item?.props.itemKey) setState(prev => ({ ...prev, activeTab: item.props.itemKey as string })); }}>
        ...
        {isConnected && clientRef.current && (
          <PivotItem headerText="Showcase" itemIcon="Rocket" itemKey="showcase">
            <OneDriveShowcase client={clientRef.current} theme={theme} onLog={addLog} userEmail={props.userEmail} userId={props.userId} />
          </PivotItem>
        )}
