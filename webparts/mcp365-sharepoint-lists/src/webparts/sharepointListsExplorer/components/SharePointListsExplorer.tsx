...
import type { ISharePointListsExplorerProps } from './ISharePointListsExplorerProps';
import {
  McpBrowserClient,
  MCP_AUDIENCE
...
import {
  LISTS_SERVER_ID,
  LISTS_SERVER_EXAMPLES
} from '../services/McpServerCatalog';
import type { IToolExample } from '../services/McpServerCatalog';
import { SharePointListsShowcase } from './SharePointListsShowcase';

// ─── Preset Storage ─────────────────────────────────────────────────

const PRESETS_STORAGE_KEY = 'mcp365-sharepoint-lists-presets';
...
export const SharePointListsExplorer: React.FC<ISharePointListsExplorerProps> = (props) => {
  ...
  const selectedExamples: IToolExample[] = state.selectedToolName
    ? (LISTS_SERVER_EXAMPLES[state.selectedToolName] || [])
    : [];
  ...
  return (
    <div style={{ padding: 16, fontFamily: theme.fonts.medium.fontFamily }}>
      ...
      <Stack horizontal verticalAlign="center" tokens={{ childrenGap: 12 }} styles={{ root: { marginBottom: 16 } }}>
        <Icon iconName="BulletedList" styles={{ root: { fontSize: 24, color: theme.palette.themePrimary } }} />
        <Text variant="xLarge" styles={{ root: { fontWeight: 600 } }}>MCP365 Explorer: SharePoint Lists</Text>
        <span style={{
          fontFamily: 'Consolas, Monaco, "Courier New", monospace',
          fontSize: 12,
          fontWeight: 600,
      ...
      <Pivot styles={{ root: { marginBottom: 16 } }} selectedKey={state.activeTab} onLinkClick={(item) => { if (item?.props.itemKey) setState(prev => ({ ...prev, activeTab: item.props.itemKey as string })); }}>
        ...
        {isConnected && clientRef.current && (
          <PivotItem headerText="Showcase" itemIcon="Rocket" itemKey="showcase">
            <SharePointListsShowcase client={clientRef.current} theme={theme} onLog={addLog} userEmail={props.userEmail} spfxContext={props.spfxContext} />
          </PivotItem>
        )}
