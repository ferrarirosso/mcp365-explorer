...
import type { ICalendarExplorerProps } from './ICalendarExplorerProps';
import {
  McpBrowserClient,
  MCP_AUDIENCE
...
const PRESETS_STORAGE_KEY = 'mcp365-calendar-presets';
...
export const CalendarExplorer: React.FC<ICalendarExplorerProps> = (props) => {
  ...
  const selectedExamples: IToolExample[] = state.selectedToolName
    ? (CALENDAR_SERVER_EXAMPLES[state.selectedToolName] || [])
    : [];
  ...
  return (
    <div style={{ padding: 16, fontFamily: theme.fonts.medium.fontFamily }}>
      ...
      <Stack horizontal verticalAlign="center" tokens={{ childrenGap: 12 }} styles={{ root: { marginBottom: 16 } }}>
        <Icon iconName="Calendar" styles={{ root: { fontSize: 24, color: theme.palette.themePrimary } }} />
        <Text variant="xLarge" styles={{ root: { fontWeight: 600 } }}>MCP365 Explorer: Outlook Calendar</Text>
        <span style={{
          fontFamily: 'Consolas, Monaco, "Courier New", monospace',
          fontSize: 12,
          fontWeight: 600,
      ...
      <Pivot styles={{ root: { marginBottom: 16 } }} selectedKey={state.activeTab} onLinkClick={(item) => { if (item?.props.itemKey) setState(prev => ({ ...prev, activeTab: item.props.itemKey as string })); }}>
        ...
        {isConnected && clientRef.current && (
          <PivotItem headerText="Showcase" itemIcon="Rocket" itemKey="showcase">
            <CalendarShowcase client={clientRef.current} theme={theme} onLog={addLog} />
          </PivotItem>
        )}
